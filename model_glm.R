...
pipeline_glm <- function(target, train_set, valid_set, test_set,
  ...
  submissions_test <- as.data.frame(cbind(
    as.character(get(paste0('pred_glm_test', suffix))) # To adjust if target is transformed
  ))
  colnames(submissions_test) <- c('target')
  submissions_test[,'target'] <- ifelse(submissions_test[,'target']=='No',0,1)
  assign(paste0('submission_glm_test', suffix),cbind(df_test$ID_code, submissions_test), envir = .GlobalEnv)
  ...
  submissions_valid[,'target'] <- ifelse(submissions_valid[,'target']=='1',0,1)
  ...
  lassoVarImp <- varImp(get(paste0('fit_glm', suffix)),scale=F)
